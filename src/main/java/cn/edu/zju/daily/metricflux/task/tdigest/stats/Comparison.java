...
public class Comparison {
     ...
     * counts in quantile ranges from each distribution and then comparing those counts using a
     * multinomial test. The result should be asymptotically chi^2 distributed if the data comes
     * from the same distribution, but this isn't so much useful as a traditional test of a null
     * hypothesis as it is just a reasonably well-behaved score that is bigger when the
     * distributions are more different, subject to having enough data to tell.
     *
     * @param dist1 First distribution (usually the reference)
     * @param dist2 Second distribution to compare (usually the test case)
     * @param qCuts The quantiles that define the bin boundaries. Values &le;0 or &ge;1 may result
     *     in zero counts. Note that the actual cuts are defined loosely as
     *     <pre>dist1.quantile(qCuts[i])</pre>
     *     .
     * @return A score that is big when dist1 and dist2 are discernibly different. A small score
     *     does not mean similarity. Instead, it could just mean insufficient data.
     */
    @SuppressWarnings("WeakerAccess")
    ...
    /**
     * Use a log-likelihood ratio test to compare two distributions. With non-linear histograms that
     * have compatible bin boundaries, all that we have to do is compare two count vectors using a
    ...
    public static double compareChi2(Histogram dist1, Histogram dist2) {
        ...
        if (n1 != k2.length
            ...
            throw new IllegalArgumentException(
                    "Incompatible histograms in terms of size or bounds");
        }
    ...
    public static double llr(double[][] count) {
        ...
        for (int i = 0; i < rows; i++) {
            for (int j = 0; j < columns; j++) {
                ...
                if (k < 0) {
                    throw new IllegalArgumentException(
                            String.format("Illegal negative count (%.5f) at %d,%d", k, i, j));
                }
                if (k > 0) {
                    h += k * Math.log(k);
                    totalCount += k;
