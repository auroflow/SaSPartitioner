...
public abstract class TDigest implements Serializable {
     ...
     *     extremely large. The number of centroids retained will be a smallish (usually less than
     *     10) multiple of this number.
    ...
    /**
     * Creates a TDigest of whichever type is the currently recommended type. MergingDigest is
     * generally the best known implementation right now.
     *
     * @param compression The compression parameter. 100 is a common value for normal uses. 1000 is
     *     extremely large. The number of centroids retained will be a smallish (usually less than
     *     10) multiple of this number.
    ...
    /**
     * Returns the fraction of all points added which are &le; x. Points that are exactly equal get
     * half credit (i.e. we use the mid-point rule)
     *
     ...
     *
     * <p>Note that this is just as expensive as actually compressing the digest. If you don't care
     * about time, but want to never over-allocate, this is fine. If you care about compression and
     * speed, you pretty much just have to overallocate by using allocating #byteSize() bytes.
     *
     * @return The number of bytes required.
     */
    ...
    /**
     * Serialize this TDigest into a byte buffer. Note that the serialization used is very
     * straightforward and is considerably larger than strictly necessary.
     *
    ...
    /**
     * Serialize this TDigest into a byte buffer. Some simple compression is used such as using
     * variable byte representation to store the centroid weights and using delta-encoding on the
     * centroid means so that floats can be reasonably used to store the centroid means.
     *
     * @param buf The byte buffer into which the TDigest should be serialized.
     */
