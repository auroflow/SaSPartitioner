...
/**
 * An AVL-tree structure stored in parallel arrays. This class only stores the tree structure, so
 * you need to extend it if you want to add data to the nodes, typically by using arrays and node
 * identifiers as indices.
 */
abstract class IntAVLTree implements Serializable {

    /** We use <tt>0</tt> instead of <tt>-1</tt> so that left(NIL) works without condition. */
    ...
    /**
     * Resize internal storage in order to be able to store data for nodes up to <code>newCapacity
     * </code> (excluded).
     */
    ...
    /**
     * Add current data to the tree and return <tt>true</tt> if a new node was added to the tree or
     * <tt>false</tt> if the node was merged into an existing node.
     */
    public boolean add() {
        if (root == NIL) {
