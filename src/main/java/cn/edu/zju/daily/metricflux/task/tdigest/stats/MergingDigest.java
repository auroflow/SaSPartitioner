...
/**
 * Maintains a t-digest by collecting new points in a buffer that is then sorted occasionally and
 ...
 * centroid which is much less than half what the AVLTreeDigest uses and no dynamic allocation is
 * required at all.
 */
public class MergingDigest extends AbstractTDigest {
    ...
    // if true, use higher working value of compression during construction, then reduce on
    // presentation
    ...
    /**
     * If you know the size of the temporary buffer for incoming points, you can use this entry
     * point.
     *
    ...
    private void mergeNewValues(boolean force, double compression) {
        ...
        if (force || unmergedWeight > 0) {
            // note that we run the merge in reverse every other merge to avoid left-to-right bias
            // in merging
            merge(
                    tempMean,
                    tempWeight,
    ...
    private void merge(
            ...
            double compression) {
        // when our incoming buffer fills up, we combine our existing centroids with the incoming
        // data,
    ...
    /**
     * Merges any pending inputs and compresses the data down to the public setting. Note that this
     * typically loses a bit of precision and thus isn't a thing to be doing all the time. It is
            ...
            if (x < mean[0]) {
                ...
                if (mean[0] - min > 0) {
                    ...
                    if (x == min) {
                        ...
                        return (1 + (x - min) / (mean[0] - min) * (weight[0] / 2 - 1))
                                / totalWeight;
                    }
                } else {
            ...
            if (x > mean[n - 1]) {
                if (max - mean[n - 1] > 0) {
                    if (x == max) {
                        ...
                        double dq =
                                (1 + (max - x) / (max - mean[n - 1]) * (weight[n - 1] / 2 - 1))
                                        / totalWeight;
                        return 1 - dq;
                    }
                } else {
