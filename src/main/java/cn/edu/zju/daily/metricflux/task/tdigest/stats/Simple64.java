...
/**
 * Very simple variable byte encoding that always uses 64bit units. The idea is that the next few
 * values are smashed into 64 bits using a few bits to indicate how they are fitted in and the rest
 * of the bits to fit each value into equal-sized chunks.
 *
 * <p>In this encoding, 4 bits are used to indicate how the remaining 60 bits are divided. The
 * possible ways are shown in the following table:
 *
 * <table>
...
public class Simple64 {
    ...
    private static final int BITS_8_MASK =
            (1 << 8) - 1; // 4 bits unused, then the last value take them
    private static final int BITS_7_MASK =
            (1 << 7) - 1; // 4 bits unused, then the last value take them
    ...
    public int add(long v, long[] out) {
        // System.out.println("S64.add v=" + v + " " + (1 + inputCount - inputCompressable) + "
        // waiting");
        ...
        while (inputCompressable <= inputCount) {
            ...
            // System.out.println("  minBits=" + minBits + " count=" + (inputCompressable-1) + "
                ...
                */
                ...
                switch (minBits) { // add status bits and later input values
                    ...
                    default:
                        ...
                        s9 = 0;
                        consumed = 60;
                        // throw new Error("S98b.compressSingle internal error: unknown minBits: " +
                        // minBits);
                }
    ...
    private static int compressSingle(
            final long[] uncompressed,
            final int inOffset,
            final int inSize,
            final LongBuffer compressedBuffer) {
        if (inSize < 1) {
            throw new IllegalArgumentException(
                    "Cannot compress input with non positive size " + inSize);
        }
        ...
        do {
            ...
            if (nextData < 0) {
                throw new IllegalArgumentException(
                        "Cannot compress negative input "
                                + nextData
                                + " (at index "
                                + (inOffset + inputCompressable - 1)
                                + ")");
            }
            while ((nextData >= maxFitPlus1) && (minBits < NUM_DATA_BITS)) {
                if ((minBits == 7) && (inputCompressable == 8) && (nextData < (maxFitPlus1 << 4))) {
    ...
    private static int decompressSingle(
            final long s9, final long[] decompressed, final int outOffset) {
        switch ((int) (s9 & 15L)) {
            ...
            default:
                throw new IllegalArgumentException(
                        "Unknown Simple9 status: " + (s9 >>> NUM_DATA_BITS));
        }
    }
    ...
    public static int decompress(LongBuffer compressedBuffer, long[] unCompressedData) {
        ...
        while (unComprSize > 0) {
            final int decoded =
                    decompressSingle(compressedBuffer.get(), unCompressedData, totalOut);
            unComprSize -= decoded;
            totalOut += decoded;
        }
        return totalOut;
