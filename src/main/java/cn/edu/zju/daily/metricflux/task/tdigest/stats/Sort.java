...
public class Sort {
    ...
    private static boolean sort(int[] order, double[] values, double[] weights, int start, int n) {
        ...
        for (int i = start; i < start + n; i++) {
            if (inOrder && i < start + n - 1) {
                inOrder =
                        values[i] < values[i + 1]
                                || (values[i] == values[i + 1] && weights[i] <= weights[i + 1]);
            }
            order[i] = i;
        }
        if (inOrder) {
    ...
    /**
     * Standard two-key quick sort on (values, weights) except that sorting is done on an index
     * array rather than the values themselves
     *
    ...
    private static void quickSort(
            int[] order, double[] values, double[] weights, int start, int end, int limit) {
        ...
        while (end - start > limit) {
            ...
            while (i < high) {
                // invariant:  (values,weights)[order[k]] == (pivotValue, pivotWeight) for k in
                // [0..low)
                // invariant:  (values,weights)[order[k]] < (pivotValue, pivotWeight) for k in
                // [low..i)
                // invariant:  (values,weights)[order[k]] > (pivotValue, pivotWeight) for k in
            ...
            // at this point, i==high, from [start,low) are == pivot, [low,high) are < and
            // [high,end) are >
            // we have to move the values equal to the pivot into the middle.  To do this, we swap
            // pivot
            // values into the top end of the [low,high) range stopping when we run out of
            // destinations
    ...
    /**
     * Stabilized quick sort on an index array. This is a normal quick sort that uses the original
     * index as a secondary key. Since we are really just sorting an index array we can do this
     * nearly for free.
     *
     * @param order The pre-allocated index array
     * @param values The values to sort
     ...
     * @param limit The minimum size to recurse down to.
     */
    private static void stableQuickSort(
    ...
    /**
     * Quick sort in place of several paired arrays. On return, keys[...] is in order and the
     * values[] arrays will be reordered as well in the same way.
     *
     ...
     * because quick sort has done its thing. This version works on parallel arrays of keys and
     * values.
     *
    ...
    public static void checkPartition(
        ...
        if (!(start >= 0 && low >= start && high >= low && end >= high)) {
            throw new IllegalArgumentException(
                    String.format("Invalid indices %d, %d, %d, %d", start, low, high, end));
        }
        ...
        for (int i = 0; i < low; i++) {
            ...
            if (v >= pivotValue) {
                throw new IllegalArgumentException(
                        String.format("Value greater than pivot at %d", i));
            }
        }
    ...
    /**
     * Limited range insertion sort with primary and secondary key. We assume that no element has to
     * move more than limit steps because quick sort has done its thing.
     *
     * <p>If weights (the secondary key) is null, then only the primary key is used.
     *
     * <p>This sort is inherently stable.
     *
    ...
    @SuppressWarnings("SameParameterValue")
    private static void insertionSort(
            int[] order, double[] values, double[] weights, int start, int n, int limit) {
        for (int i = start + 1; i < n; i++) {
            ...
            for (int j = i; j >= m; j--) {
                if (j == 0
                        || values[order[j - 1]] < v
                        || (values[order[j - 1]] == v
                                && (weights == null || weights[order[j - 1]] <= w))) {
                    if (j < i) {
    ...
    /**
     * Limited range insertion sort with primary key stabilized by the use of the original position
     * to break ties. We assume that no element has to move more than limit steps because quick sort
     * has done its thing.
     *
     * @param order The permutation index
     * @param values The values we are sorting
     ...
     */
    @SuppressWarnings("SameParameterValue")
    private static void stableInsertionSort(
            int[] order, double[] values, int start, int n, int limit) {
        for (int i = start + 1; i < n; i++) {
            int t = order[i];
            double v = values[order[i]];
            int vi = order[i];
            ...
            for (int j = i; j >= m; j--) {
                if (j == 0
                        || values[order[j - 1]] < v
                        || (values[order[j - 1]] == v && (order[j - 1] <= vi))) {
                    if (j < i) {
                        System.arraycopy(order, j, order, j + 1, i - j);
                        order[j] = t;
                    }
