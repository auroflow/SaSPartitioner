...
import static cn.edu.zju.daily.metricflux.task.tdigest.stats.IntAVLTree.NIL;
...
public class AVLTreeDigest extends AbstractTDigest {
     ...
     *
     * @param compression How should accuracy be traded for size? A value of N here will give
     *     quantile errors almost always less than 3/N with considerably smaller errors expected for
     *     extreme quantiles. Conversely, you should expect to track about 5 N centroids for this
     *     accuracy.
     */
    ...
    /**
     * Sets the seed for the RNG. In cases where a predicatable tree should be created, this
     * function may be used to make the randomness in this AVLTree become more deterministic.
     *
    ...
    public TDigest recordAllData() {
        if (summary.size() != 0) {
            throw new IllegalStateException(
                    "Can only ask to record added data on an empty summary");
        }
    ...
    public void add(double x, int w, List<Double> data) {
        ...
        if (start == NIL) { // empty summary
            ...
            int closest = NIL;
            double n = 0;
            for (int neighbor = start;
                    neighbor != lastNeighbor;
                ...
                double k =
                        count
                                * Math.min(
                                        scale.max(q0, compression, count),
                                        scale.max(q1, compression, count));

                // this slightly clever selection method improves accuracy with lots of repeated
                // points
                ...
                if (x < bMean) {
                    ...
                    if (aWeight == 1) {
                        ...
                        if (bWeight == 1) {
                            ...
                            // only get to interpolate b's weight because a is a singleton and to
                            // our left
                        ...
                        double partialWeight =
                                (x - aMean) / (bMean - aMean) * (aWeight + bWeight) / 2.0;
                        return (weightSoFar + aWeight / 2.0 + partialWeight) / size();
                    }
                }
                weightSoFar += aWeight;
    ...
    private double interpolateTail(
            AVLGroupTree values, double x, int node, double mean, double extremeValue) {
    ...
    /**
     * Returns an upper bound on the number of bytes that will be required to represent this
     * histogram in the tighter representation.
     */
    @Override
