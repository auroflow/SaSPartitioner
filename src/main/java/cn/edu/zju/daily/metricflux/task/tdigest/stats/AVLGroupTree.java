...
final class AVLGroupTree extends AbstractCollection<Centroid> implements Serializable {
                ...
                new IntAVLTree() {
                    ...
                    protected void fixAggregates(int node) {
                        ...
                        aggregatedCounts[node] =
                                counts[node]
                                        + aggregatedCounts[left(node)]
                                        + aggregatedCounts[right(node)];
                    }
                };
    ...
    public void update(
            int node, double centroid, int count, List<Double> data, boolean forceInPlace) {
    ...
    /**
     * Return the last node so that the sum of counts of nodes that are before it is less than or
     * equal to <code>sum</code>.
     */
    @SuppressWarnings("WeakerAccess")
    ...
    /**
     * Compute the number of elements and sum of counts for every entry that is strictly before
     * <code>node</code>.
     */
    @SuppressWarnings("WeakerAccess")
    ...
    private void checkAggregates(int node) {
        assert aggregatedCounts[node]
                == counts[node]
                        + aggregatedCounts[tree.left(node)]
                        + aggregatedCounts[tree.right(node)];
