...
/**
 * Non-linear histogram that uses floating point representation plus a quadratic correction to bin
 * width to achieve tighter fit to the ideal log2 sizing.
 */
public class LogHistogram extends Histogram {
    ...
    public LogHistogram(double min, double max, double epsilonFactor) {
        ...
        if (max <= 2 * min) {
            throw new IllegalArgumentException(
                    String.format("Illegal/nonsensical min, max (%.2f, %.2g)", min, max));
        }
        if (min <= 0 || max <= 0) {
    ...
    /**
     * Approximates log_2(value) by abusing floating point hardware. The floating point exponent is
     * used to get the integer part of the log. The mantissa is then adjusted with a second order
    ...
    public static double approxLog2(double value) {
        ...
        final double m =
                Double.longBitsToDouble((valueBits & 0x800fffffffffffffL) | 0x3ff0000000000000L);
        return (m * (2 - (1.0 / 3) * m) + exponent - (2.0 / 3.0));
    }

    /**
     * Computes an approximate value of 2^x. This is done as an exact inverse of #approxLog2 so that
     * bin boundaries can be computed exactly.
     *
    ...
    void add(Iterable<Histogram> others) {
        for (Histogram other : others) {
            if (!this.getClass().equals(other.getClass())) {
                throw new IllegalArgumentException(
                        String.format("Cannot add %s to LogHistogram", others.getClass()));
            }
            LogHistogram actual = (LogHistogram) other;
            if (actual.min != min || actual.max != max || actual.counts.length != counts.length) {
                throw new IllegalArgumentException(
                        "Can only merge histograms with identical bounds and precision");
            }
