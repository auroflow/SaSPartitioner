...
/**
 * Maintains histogram buckets that are constant width in base-2 floating point representation
 * space. This is close to exponential binning, but should be much faster.
 */
    ...
    double lowerBound(int k) {
        return min
                * Double.longBitsToDouble(
                        (k + (0x3ffL << bitsOfPrecision)) << (52 - bitsOfPrecision)) /* / fuzz */;
    }
    ...
    void add(Iterable<Histogram> others) {
        for (Histogram other : others) {
            if (!this.getClass().equals(other.getClass())) {
                throw new IllegalArgumentException(
                        String.format("Cannot add %s to FloatHistogram", others.getClass()));
            }
