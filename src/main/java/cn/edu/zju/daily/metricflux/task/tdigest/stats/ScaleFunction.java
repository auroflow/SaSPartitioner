...
package cn.edu.zju.daily.metricflux.task.tdigest.stats;

/**
 * Encodes the various scale functions for t-digests. These limits trade accuracy near the tails
 * against accuracy near the median in different ways. For instance, K_0 has uniform cluster sizes
 * and results in constant accuracy (in terms of q) while K_3 has cluster sizes proportional to
 * min(q,1-q) which results in very much smaller error near the tails and modestly increased error
 * near the median.
 *
 * <p>The base forms (K_0, K_1, K_2 and K_3) all result in t-digests limited to a number of clusters
 * equal to the compression factor. The K_2_NO_NORM and K_3_NO_NORM versions result in the cluster
 * count increasing roughly with log(n).
 */
public enum ScaleFunction {
    ...
    /**
     * Generates cluster sizes proportional to sqrt(q*(1-q)). This gives constant relative accuracy
     * if accuracy is proportional to squared cluster size. It is expected that K_2 and K_3 will
    ...
    /**
     * Generates cluster sizes proportional to sqrt(q*(1-q)) but avoids computation of asin in the
     * critical path by using an approximate version.
     */
    K_1_FAST {
        @Override
    ...
    /**
     * Generates cluster sizes proportional to q*(1-q). This makes tail error bounds tighter than
     * for K_1. The use of a normalizing function results in a strictly bounded number of clusters
    ...
    /**
     * Generates cluster sizes proportional to min(q, 1-q). This makes tail error bounds tighter
     * than for K_1 or K_2. The use of a normalizing function results in a strictly bounded number
     * of clusters no matter how many samples.
     */
    K_3 {
        @Override
        public double k(double q, final double compression, final double n) {
    ...
    /**
     * Generates cluster sizes proportional to q*(1-q). This makes the tail error bounds tighter.
     * This version does not use a normalizer function and thus the number of clusters increases
     * roughly proportional to log(n). That is good for accuracy, but bad for size and bad for the
     * statically allocated MergingDigest, but can be useful for tree-based implementations.
     */
    K_2_NO_NORM {
        @Override
        public double k(double q, final double compression, double n) {
    ...
    /**
     * Generates cluster sizes proportional to min(q, 1-q). This makes the tail error bounds
     * tighter. This version does not use a normalizer function and thus the number of clusters
     * increases roughly proportional to log(n). That is good for accuracy, but bad for size and bad
     * for the statically allocated MergingDigest, but can be useful for tree-based implementations.
     */
    K_3_NO_NORM {
        @Override
        public double k(double q, final double compression, final double n) {
    ...
    /**
     * Converts a quantile to the k-scale. The total number of points is also provided so that a
     * normalizing function can be computed if necessary.
     *
    ...
    /**
     * Converts a quantile to the k-scale. The normalizer value depends on compression and
     * (possibly) number of points in the digest. #normalizer(double, double)
     *
     * @param q The quantile
    ...
    /**
     * Computes q as a function of k. This is often faster than finding k as a function of q for
     * some scales.
     *
     ...
     * to 1 into 5 regions for all but the region nearest 1, rational polynomial models get us a
     * very good approximation of asin and by interpolating as we move from region to region, we can
     * guarantee continuity and we happen to get monotonicity as well. for the values near 1, we
