...
def set_config(config: AlgorithmConfig, conf: Config, env_class, env_config):
    return (
        config.api_stack(
        ...
        .env_runners(
            ...
            num_cpus_per_env_runner=max(1, conf.num_cpus - 1),
...
def get_sac_n_config(
    env_class: Union[str, Type[gym.Env]],
    conf: Config,
    ...
    return (
        set_config(SACConfig(), conf, env_class, env_config),
        SAC,
    )
