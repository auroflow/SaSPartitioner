...
from ray.rllib.algorithms import SACConfig, CQLConfig
...
from ray.rllib.utils.typing import ModuleID, TensorType, ParamDict
...
class CQLNTorchLearner(SACNTorchLearner):
    ...
    def compute_loss_for_module(
        ...
        *,
        ...
        # Calculate the TD error: [ensemble_size, batch_size]
        ...
        random_density = torch.log(
            torch.pow(
                1,  # originally 1/2, we change to 1 because high = 1, low = 0
                torch.tensor(
        ...
        # dim: [ensemble_size, batch_size, num_actions, 1]
        q_rand_repeat = fwd_out["q_rand_repeat"] - random_density
        q_next_repeat = fwd_out["q_next_repeat"] - fwd_out["logps_next_repeat"].detach()
        q_curr_repeat = fwd_out["q_curr_repeat"] - fwd_out["logps_curr_repeat"].detach()

        q_repeat = torch.cat(
            [q_rand_repeat, q_next_repeat, q_curr_repeat],
            dim=2,
        )

        # dim: [ensemble_size]
        cql_losses = (
            torch.logsumexp(q_repeat / config.temperature, dim=2).mean(dim=(1, 2))
            * config.min_q_weight
            * config.temperature
        )
        cql_losses -= q_selected.mean(dim=1) * config.min_q_weight
        # Add the CQL loss term to the SAC loss term. Dim: [ensemble_size]
        critic_losses = sac_critic_losses + cql_losses

        # TODO (simon): Check, if we need to implement here also a Lagrangian
        # loss.
        # TODO: original implementation uses sum in SAC but mean in CQL. Check which is better.
        ...
        self.metrics.log_dict(
            {
                ...
                "cql_loss": torch.sum(cql_losses),
                "cql_loss_1": cql_losses[0],
                "cql_loss_2": cql_losses[1],
